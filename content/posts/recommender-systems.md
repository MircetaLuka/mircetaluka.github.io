...
$$
Pearson (u, v) = \frac{\sum\_{i \in I\_{uv}} (r\_{u,i} - \bar{r}\_u)(r\_{v,i} - \bar{r}\_v)}{\sqrt{\sum\_{i \in I\_{uv}} (r\_{u,i} - \bar{r}\_u)^2} \sqrt{\sum\_{i \in I\_{uv}} (r\_{v,i} - \bar{r}\_v)^2}}
$$

where $r\_{u,i}$ is the rating given by user $u$ to item $i$, $r\_{v,i}$ is the rating given by user $v$ to item $i$, $\bar{r}\_u$ is the average rating of user $u$, $\bar{r}\_v$ is the average rating of user $v$, and $I\_{uv}$ is the set of items rated by both users $u$ and $v$.
